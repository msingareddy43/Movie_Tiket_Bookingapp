...
def delete_movie_type(request,pid):
    ...
    data = Movie_Type.objects.get(id=pid)
    data.delete()
    return redirect('view_movie_type')
...
def Book_Ticket(request, pid):
    ...
    data = get_object_or_404(Set_Timing, id=pid)
    data1 = Booking.objects.filter(set_time=data)
    li = ""
    for i in data1:
        li += "," + i.seat
    print("Reserved seats:", li)  # Debug statement
    error = False
    book = None
    ...
    if request.method == "POST":
        ...
        n = request.POST['num']
        s = request.POST['seat']
        print("Seat entered by user:", s)  # Debug statement
        p = int(n) * 120
        ...
        try:
            with transaction.atomic():
                ...
                reserved = Booking.objects.select_for_update().filter(set_time=data, seat=s).exists()
                print("Is seat reserved?", reserved)  # Debug statement
                if reserved:
                    error = True
                    messages.error(request, 'Seat is already reserved. Please choose another seat.')
                else:
                    book = Booking.objects.create(user1=cust, user=t, set_time=data, ticket=n, price=p, seat=s)
                    return redirect('payment', pid=book.id)  # Redirect to payment URL
        except Exception as e:
            print(f"Booking failed: {e}")  # Debug statement
            error = True
            messages.error(request, 'An error occurred during the booking process. Please try again.')
...
def All_Movies(request,pid):
    ...
    if pid == 3:
        ...
        lang = Language.objects.get(name="Telugu")
        data = Movie.objects.filter(language=lang)
...
def delete_bookings(request,pid):
    if not request.user.is_authenticated:
        return redirect('login')
